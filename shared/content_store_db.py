...
Supports both Flask app context (via shared.db) and standalone usage
...
class DbContentReader:
    """Reads content from PostgreSQL tables.

    When flask_app is provided, uses Flask app context connection pool.
    When flask_app is None, uses standalone connection pool.
    """
    ...
    def read(self, filename: str):
        if self._flask_app is not None:
            cursor = get_dict_cursor(self._flask_app)
            try:
                return self._read_file(cursor, filename)
            finally:
                cursor.close()
        else:
            with get_dict_cursor_ctx() as (cursor, conn):
                return self._read_file(cursor, filename)
...
class DbContentWriter:
    """Writes content to PostgreSQL tables.
...
def get_content_reader(flask_app=None) -> DbContentReader:
    """Get a content reader instance.

    Args:
        flask_app: Flask app instance for app-context usage, or None for standalone.
    """
    return DbContentReader(flask_app)


def get_content_writer(flask_app=None) -> DbContentWriter:
    """Get a content writer instance.

    Args:
        flask_app: Flask app instance for app-context usage, or None for standalone.
    """
    return DbContentWriter(flask_app)
