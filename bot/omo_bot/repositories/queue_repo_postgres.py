"""PostgreSQL queue repository implementation."""
...
from .queue_repo_common import row_to_entry, row_to_event, row_to_summary
...
class PostgresQueueRepository:
    ...
    def list_queues(self) -> tuple[QueueSummary, ...]:
        ...
        try:
            ...
            return tuple(row_to_summary(row) for row in cursor.fetchall() or [])
        finally:
            cursor.close()
            connection.close()
    ...
    def get_queue(self, queue_id: str) -> QueueSnapshot | None:
        ...
        try:
            ...
            return QueueSnapshot(
                summary=row_to_summary(summary_row),
                entries=tuple(row_to_entry(row) for row in entry_rows),
