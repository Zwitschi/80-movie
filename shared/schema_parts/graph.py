from shared.db import get_dict_cursor
from shared.movie_data import PRODUCTION_COMPANY_NAME
...
def _resolve_movie_id(movie):
    ...
    if isinstance(movie, dict) and movie.get('title'):
        cursor.execute("SELECT id FROM movie WHERE title = %s",
                       (movie['title'],))
    else:
...
def build_movie_schema_graph(movie):
    ...
    organization_node = build_organization_node(
