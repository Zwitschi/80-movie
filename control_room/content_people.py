...
def _render_people_form(*, save_error, save_success, people, contributors, credits_people, organizations, page_context):
    ...
    return render_template(
        ...
        contributor_sections=admin_content.CONTRIBUTOR_SECTIONS,
        **page_context,
    )
...
def _handle_people_request(request):
    ...
    people_payload = admin_content.load_json('people.json')
    orgs_payload = admin_content.load_json('organizations.json')
    ...
    if request.method == 'POST':
        ...
        if action == 'add_person':
            entry, err = admin_content._person_from_form(request.form, prefix='person_')
            if err:
                save_error = err
            else:
                people = dict(people)
                ...
                success, save_error = admin_content.save_json('people.json', people_payload)
                if success:
            ...
            if section not in admin_content.CONTRIBUTOR_SECTIONS:
                save_error = f'Invalid contributor section: {section!r}.'
            else:
                entry, err = admin_content._contributor_from_form(
                    request.form,
                    prefix='contributor_',
                )
                if err:
                    ...
                    contributors[section] = admin_content.process_list_action(
            ...
            entry, err = admin_content._credit_from_form(request.form, prefix='credit_')
            if err:
                save_error = err
            else:
                credits_people = admin_content.process_list_action(
                    credits_people,
                    'add',
                    '',
                    entry,
            ...
            entry, err = admin_content._org_from_form(request.form, prefix='org_')
            if err:
                save_error = err
            else:
                ...
                success, save_error = admin_content.save_json('organizations.json', orgs_payload)
                if success:
                    return redirect(url_for('content.edit_people', saved='1'))
